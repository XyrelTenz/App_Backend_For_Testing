 ...
 *
 * Tracks failed authentication attempts per identifier (email / IP).
 * After [maxAttempts] failures within [windowSeconds], blocks further attempts.
 * Block auto-expires after [blockDurationSeconds].
 */
@Component
class BruteForceProtectionService(
    @param:Value("\${" + "security.brute-force.max-attempts:5}") private val maxAttempts: Int,
    @param:Value("\${" + "security.brute-force.block-duration-seconds:900}") private val blockDurationSeconds: Long,
     ...
     * @throws UnauthorizedException if blocked.
     */
    ...
    fun recordFailure(identifier: String) {
        ...
        if (newCount >= maxAttempts) {
            ...
            log.warn("Blocking [{}] for {} seconds after {} failed attempts", identifier, blockDurationSeconds, newCount)
        }
    }
}
